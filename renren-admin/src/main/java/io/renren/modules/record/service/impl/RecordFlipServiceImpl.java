...
import io.renren.modules.record.dao.RecordFlipDao;
 ...
 */
@Service
public class RecordFlipServiceImpl extends CrudServiceImpl<RecordFlipDao, RecordFlipEntity, RecordFlipDTO> implements RecordFlipService {

    @Override
    public QueryWrapper<RecordFlipEntity> getWrapper(Map<String, Object> params){
        String id = (String)params.get("id");

        QueryWrapper<RecordFlipEntity> wrapper = new QueryWrapper<>();
        wrapper.eq(StringUtils.isNotBlank(id), "id", id);

        return wrapper;
    }
    @Override
    public void save(RecordFlipDTO dto){
        // 保存该条记录
        super.save(dto);
        // 修改对应学生记录数
...
//            wrapper.eq("id",id);
            RecordFlipDTO dto = super.get(id);
    ...
    @Override
    //测试函数
    public List<RecordFlipDTO> getByStuId(Integer stuId){
        QueryWrapper<RecordFlipEntity> wrapper = new QueryWrapper<>();
        //
        wrapper.eq("student_id",stuId);
        List<RecordFlipEntity> entityList = baseDao.selectList(wrapper);
        return ConvertUtils.sourceToTarget(entityList,RecordFlipDTO.class);
    }
